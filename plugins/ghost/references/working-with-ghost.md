...
Context definitions are supported resources: `/data/icps`, `/data/personas`, `/data/products`, `/data/use_cases`, `/data/competitors`, `/data/case_studies`, `/data/proof_points`, `/data/resources`, `/data/context_folders`, and `/data/context_documents`. Read an individual definition at `/data/{resource}/{id}`. Use `context.get_workspace_context` for workspace context/overview and `context.apply_entity` for supported edits. Do not guess `/icps` or `/signals`; discover signal operations in `/capabilities`.

Stored graph/CRM-mirror data requires `graph:read` and is not necessarily current at the CRM. Direct live HubSpot company, contact, and deal property reads use `crm.read_hubspot_company_properties`, `crm.read_hubspot_contact_properties`, and `crm.read_hubspot_deal_properties` through `/query`, with `crm:read`. Discover exact custom/base property keys before reading. For filtered connected reads use `graph.query_connected_api`; HubSpot requires both its normal `graph:read` and additional `crm:read`. Live CRM results identify their source; preserve missing IDs, coverage and reconciliation conflicts.

`crm:read` and `crm:write` are independent grants. CRM write permits supported confirmed CRM mutations; it does not imply live-read permission. Graph write permits Ghost edits, not CRM edits. Old keys are not silently expanded: a key needs an explicit `crm:read` grant to use the new live-read policy. CRM field searches require the same grant; the sources-only catalog is available without it. Workflow permissions separately authorize the workflow's approved automation, which can include CRM effects; review that definition and its destinations before granting/using `workflows:run`.

For delegated population use `graph-agent`. Claude creates the exact account/field/context/query plan and obtains scoped execution and credit-cap approval. Gemini then executes reads and edits without asking for another approval per suggested value. It can add selected matches and fill the supported fields, not invent new tools or expand the cohort. Its model charges are user-requested agent work, separately attributed from ingestion and normal API fees. A preview does no model work; it remains a successful API call with the standard fee.
